...
namespace Parking_Api.Hubs
{
    public class ParkingHub : Hub
    {
        ...
        private async Task SendParkingSessionWarnings(int userId)
        {
            ...
            var sessions = await _contextDb.ParkingSessions.Include(x => x.car).Include(x => x.parkingComplex).Include(x => x.parkingSpot)
                .Where(x => x.user_id == userId && x.status == "Активна" && x.exit_time != null && x.exit_time > now && x.exit_time <= warningLimit).ToListAsync();
